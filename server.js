...
async function getChatGPTResponse(userMessage, conversationHistory = [], companyInfo = ZULU_CLUB_INFO) {
  ...
  try {
    ...
    // If GPT didn't return "product" but history suggests product -> force product flow
    if (intent !== 'product') {
      const historyHasProduct = recentHistoryContainsProductSignal(conversationHistory);
      if (historyHasProduct) {
        console.log('ℹ️ Classifier did not say product but session history contains product-like messages -> forcing product flow.');
        intent = 'product';
      }
    }
    ...
    if (intent === 'product' && galleriesData.length > 0) {
      ...
      // If no matches from classifier, use GPT matcher with conversation history included
...
/* -------------------------
   ZULU CLUB INFORMATION (updated to include download links)
--------------------------*/
const ZULU_CLUB_INFO = `
We're building a new way to shop and discover lifestyle products online.
...
function buildConciseResponse(userMessage, galleryMatches = [], sellersObj = {}) {
  ...
  // Append download links for every product response
  msg += DOWNLOAD_LINKS;
...
/* -------------------------
   ...
   Now accepts conversationHistory for context
--------------------------*/
async function findGptMatchedCategories(userMessage, conversationHistory = []) {
  try {
...
/* -------------------------
   ...
   (MODIFIED to accept conversationHistory and include it in the prompt)
--------------------------*/
async function classifyAndMatchWithGPT(userMessage, conversationHistory = []) {
  const text = (userMessage || '').trim();
  if (!text) {
...
async function generateCompanyResponse(userMessage, conversationHistory, companyInfo) {
  ...
  try {
    ...
    const reply = completion.choices[0].message.content.trim();
    // append download links so any company response includes app links
    return `${reply}${DOWNLOAD_LINKS}`;
  } catch (e) {
    console.error('Error in generateCompanyResponse:', e);
    return `Hi! We're Zulu Club — shop at zulu.club or visit our pop-ups in Gurgaon.${DOWNLOAD_LINKS}`;
...
async function getChatGPTResponse(userMessage, conversationHistory = [], companyInfo = ZULU_CLUB_INFO) {
  ...
  try {
    ...
    // 1) single GPT call to classify + match categories (now receives conversationHistory)
    const classification = await classifyAndMatchWithGPT(userMessage, conversationHistory);
    ...
    if (intent === 'investors') {
      // Return investors paragraph (editable) + download links
    ...
    if (intent === 'product' && galleriesData.length > 0) {
      ...
      const sellers = await findSellersForQuery(userMessage, matchedCategories, detectedGender);
      // buildConciseResponse appends DOWNLOAD_LINKS
...
function generateFallbackProductResponse() {
  return `🎉 Zulu Club offers many categories — visit zulu.club to browse.${DOWNLOAD_LINKS}`;
}
...
async function handleMessage(sessionId, userMessage) {
  try {
    if (!conversations[sessionId]) conversations[sessionId] = { history: [], lastActive: nowMs() };
    appendToSessionHistory(sessionId, 'user', userMessage);
    const fullHistory = getFullSessionHistory(sessionId);
    const aiResponse = await getChatGPTResponse(userMessage, fullHistory);
    appendToSessionHistory(sessionId, 'assistant', aiResponse);
...
app.get('/', (req, res) => {
  res.json({ 
    status: 'Server is running on Vercel', 
    service: 'Zulu Club WhatsApp AI Assistant',
    version: '6.0 - GPT-first classifier & category matcher (AI is boss) - category-driven gender - download links added',
    stats: {
