...
// Gender detection uses category data (cat_id / cat1) as the source of truth.
...
/* -------------------------
   ZULU CLUB INFO & INVESTORS PARAGRAPH
--------------------------*/
...
Now live in Gurgaon. Visit zulu.club for more.
`;
...
/* -------------------------
   Google Sheets helpers (best-effort)
--------------------------*/
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      stream.pipe(csv())
        ...
        .on('end', () => { console.log(`✅ Loaded ${results.length} product categories`); resolve(results); })
        .on('error', (error) => { console.error('❌ Error parsing CSV:', error); reject(error); });
    });
  } catch (error) { console.error('❌ Error loading CSV data:', error.message); return []; }
}
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      stream.pipe(csv())
        ...
        .on('end', () => { console.log(`✅ Loaded ${results.length} sellers`); resolve(results); })
        .on('error', (error) => { console.error('❌ Error parsing sellers CSV:', error); reject(error); });
    });
  } catch (error) { console.error('❌ Error loading sellers CSV:', error.message); return []; }
}

(async () => {
  try { galleriesData = await loadGalleriesData(); } catch (e) { galleriesData = []; console.error(e); }
  try { sellersData = await loadSellersData(); } catch (e) { sellersData = []; console.error(e); }
})();
...
/* -------------------------
   Small GPT helpers (isQueryHome, gptCheckSellerMaySell)
   These call OpenAI and expect strict JSON back.
--------------------------*/
...
async function findSellersForQuery(userMessage, galleryMatches = [], detectedGender = null) {
  ...
  if (applyHomeFilter) {
    const homeSyns = ['home','decor','home decor','furniture','lamp','vase','clock','showpiece','cushion'];
    const keepIfHome = (s) => {
  ...
  if (candidateIds.size === 0) {
    if (applyHomeFilter) {
      for (const s of sellersData) {
        const arr = s.category_ids_array || [];
        if (arr.some(c => c.includes('home') || c.includes('decor') || c.includes('furnit') || c.includes('lamp') || c.includes('vase') || c.includes('clock'))) {
...
/* -------------------------
   findGptMatchedCategories (history-aware, but called only after intent is 'product')
--------------------------*/
...
RESPONSE FORMAT (JSON only):
{
  "matches": [
    ...
    console.log(`🧾 findGptMatchedCategories -> sending ${messagesForGPT.length} messages to OpenAI (history included).`);
    const completion = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
    ...
    const matchedCategories = matches.map(match => galleriesData.find(item => item.type2 === match.type2)).filter(Boolean).slice(0,5);
...
/* -------------------------
   CLASSIFIER: classifyAndMatchWithGPT (single-message)
--------------------------*/
...
2) If the intent is "product", pick up to 5 best-matching categories from the AVAILABLE CATEGORIES list provided (use "type2" field). For each match return a short reason and a relevance score between 0.0 and 1.0.
...
"""
  ...
  try {
    const completion = await openai.chat.completions.create({
      model: "gpt-3.5-turbo",
      messages: [{ role: "system", content: "You are a JSON-only classifier & category matcher. Return only the requested JSON." }, { role: "user", content: prompt }],
      max_tokens: 800,
      temperature: 0.15
    });
...
/* -------------------------
   ...
   - If includeAppLinks = true, append APP_LINKS_TEXT at the end.
   - If greeting (short greeting) we will call with includeAppLinks = false.
--------------------------*/
...
async function generateCompanyResponse(userMessage, conversationHistory, includeAppLinks = true, companyInfo = ZULU_CLUB_INFO) {
  const messages = [];
  const systemMessage = {
    role: "system",
...
3. Mention availability (popups in Gurgaon) when relevant
4. Use emojis lightly
5. Keep responses under 200 characters when possible for WhatsApp
...
`
  ...
  try {
    ...
    if (includeAppLinks) reply = `${reply}${APP_LINKS_TEXT}`;
    return reply;
  } catch (e) {
    console.error('Error in generateCompanyResponse:', e);
    return `${companyInfo.trim()}${includeAppLinks ? APP_LINKS_TEXT : ''}`;
  }
}
...
const MAX_HISTORY_MESSAGES = 2000; // keep lots of messages within the hour
...
function createOrTouchSession(sessionId) {
  ...
  } else conversations[sessionId].lastActive = nowMs();
...
function appendToSessionHistory(sessionId, role, content) {
  ...
  if (conversations[sessionId].history.length > MAX_HISTORY_MESSAGES) conversations[sessionId].history = conversations[sessionId].history.slice(-MAX_HISTORY_MESSAGES);
...
app.get('/session/:id', (req, res) => {
  ...
  res.json({ sessionId: id, lastActive: s.lastActive, lastDetectedIntent: s.lastDetectedIntent, lastDetectedIntentTs: s.lastDetectedIntentTs, historyLen: s.history.length, history: s.history });
});
...
function recentHistoryContainsProductSignal(conversationHistory = []) {
  ...
  const productKeywords = ['tshirt','t-shirt','shirt','tee','jeans','pant','pants','trouser','kurta','lehenga','top','dress','saree','jacket','sweater','shorts'];
  const recentUserMsgs = conversationHistory.slice(-10).filter(m => m.role === 'user').map(m => (m.content || '').toLowerCase());
  for (const msg of recentUserMsgs) {
    for (const pk of productKeywords) if (msg.includes(pk)) return true;
  }
  return false;
}

/* -------------------------
   SESSION-AWARE getChatGPTResponse
   - classify single-message only
   - override to 'product' for short qualifiers if prior product intent within TTL
   - after intent set to product, call findGptMatchedCategories with full history
--------------------------*/
function isShortOrQualifier(msg) {
  if (!msg) return false;
...
async function getChatGPTResponse(sessionId, userMessage, companyInfo = ZULU_CLUB_INFO) {
  ...
  try {
    ...
    // Quick explicit onboarding check
    ...
    // If classifier says product -> update session
    if (intent === 'product') {
      session.lastDetectedIntent = 'product';
      session.lastDetectedIntentTs = nowMs();
    }

    // Override rule: if classifier not product but session had recent product intent within TTL & current message is short qualifier -> treat as product
    ...
    if (intent !== 'product' && lastProductWithinTTL && isShortOrQualifier(userMessage)) {
      console.log('ℹ️ Overriding to product due to recent product intent and short qualifier.');
      intent = 'product';
      // keep session.lastDetectedIntentTs unchanged to maintain continuity
    }
    ...
    if (intent === 'product' && galleriesData.length > 0) {
      ...
      // Use classifier-provided matches first
      ...
      // If no matches or refinement needed, call findGptMatchedCategories with full history
      if (matchedCategories.length === 0 || isShortOrQualifier(userMessage)) {
    ...
    // Default: company response. If greeting, no app links; otherwise append APP_LINKS_TEXT
    const greeting = isGreetingMessage(userMessage);
    return await generateCompanyResponse(userMessage, getFullSessionHistory(sessionId), !greeting, companyInfo);
...
/* -------------------------
   handleMessage (session-aware) - saves history, logs to sheets
--------------------------*/
async function handleMessage(sessionId, userMessage) {
  try {
    ...
    fullHistory.forEach((h, idx) => console.log(`   ${idx + 1}. [${h.role}] ${h.content}`));
...
app.get('/', (req, res) => {
  res.json({
    ...
    version: '7.0 - Intent-first, session-aware (1 hour), history used only after intent',
...
app.get('/refresh-csv', async (req, res) => {
  try { galleriesData = await loadGalleriesData(); sellersData = await loadSellersData(); res.json({ status: 'success', message: 'CSV refreshed', categories_loaded: galleriesData.length, sellers_loaded: sellersData.length }); }
...
app.get('/test-gpt-matching', async (req, res) => {
  const { query } = req.query; if (!query) return res.status(400).json({ error: 'Missing query parameter' });
  try {
    const dummyHistory = [{ role: 'user', content: 'Earlier I asked about tshirts' }, { role: 'assistant', content: 'Would you like men or women?' }];
