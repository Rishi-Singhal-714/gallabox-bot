...
/* -------------------------
   ...
   - Now includes recent session messages as context, so follow-ups like
     "tshirt" then "men" map correctly.
--------------------------*/
...
/* -------------------------
   ...
   - classifier now sees recent messages and must use them
--------------------------*/
...
/* -------------------------
   ...
   (fallback when classifier misses product intent)
--------------------------*/
function recentHistoryContainsProductSignal(conversationHistory = []) {
  ...
  // product keywords (expandable)
  const productKeywords = ['tshirt','t-shirt','shirt','tee','jeans','pant','pants','trouser','kurta','lehenga','top','dress','saree','innerwear','jacket','sweater','shorts','t-shirt','tshir','t shrt','t shrt'];
...
/* -------------------------
   Main product flow:
   - We now pass full conversationHistory into classifier & matcher.
   - If classifier says product -> use its matches
   - If classifier doesn't say product but recent history contains product keywords -> force product flow using findGptMatchedCategories over combined prior user messages + latest
--------------------------*/
