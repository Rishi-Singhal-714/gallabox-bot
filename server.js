...
/* -------------------------
   ...
   - Sessions created per phone number (sessionId)
   - All messages appended to session.history (user + assistant)
   - Session kept alive on each message (lastActive updated)
   - Session removed ONLY after SESSION_TTL_MS of inactivity
   - We pass the ENTIRE session.history to GPT calls while session exists
   - To avoid runaway memory we cap messages with MAX_HISTORY_MESSAGES
--------------------------*/
...
function appendToSessionHistory(sessionId, role, content) {
  ...
  // safety cap only (we DO NOT trim older messages automatically)
  if (conversations[sessionId].history.length > MAX_HISTORY_MESSAGES) {
    // keep the latest MAX_HISTORY_MESSAGES entries
...
// Optional debug endpoint: view session (remove or protect in production)
...
/* -------------------------
   Small/concise response builder (kept)
--------------------------*/
...
/* -------------------------
   ...
   NOTE: we will pass full session.history into this when appropriate
--------------------------*/
async function generateCompanyResponse(userMessage, conversationHistory, companyInfo) {
  ...
  if (conversationHistory && conversationHistory.length > 0) {
    // conversationHistory expected to be the FULL session.history array
    // include last up to 6 messages to reduce token usage in the assistant response
...
/* -------------------------
   Main product flow:
   - When handling a message we will:
     1) create/touch session
     2) append user message to session.history
     3) pass the FULL session.history to getChatGPTResponse so GPT can use all prior messages
     4) append assistant response to history
     5) do NOT clear history until session TTL expires
--------------------------*/
...
/* -------------------------
   Webhook + endpoints
   (handleMessage wired to session/back-history logic)
--------------------------*/
async function handleMessage(sessionId, userMessage) {
  try {
    // 1) Ensure session exists and append user message (full history persists)
    appendToSessionHistory(sessionId, 'user', userMessage);

    // 2) Retrieve FULL session history and pass it to getChatGPTResponse
    const fullHistory = getFullSessionHistory(sessionId);

    // 3) Get AI response (GPT functions that use conversationHistory will receive the full history)
    const aiResponse = await getChatGPTResponse(userMessage, fullHistory);

    // 4) Append assistant response to session history (full persisted)
    appendToSessionHistory(sessionId, 'assistant', aiResponse);

    // 5) update lastActive (appendToSessionHistory already set lastActive), do not clear history here
    conversations[sessionId].lastActive = nowMs();
...
// --- SESSION / BACK-HISTORY LOGIC (ADDED) ---
const SESSION_TTL_MS = 1000 * 60 * 60; // 1 hour
const SESSION_CLEANUP_MS = 1000 * 60 * 5; // cleanup every 5 minutes
const MAX_HISTORY_MESSAGES = 50; // max messages stored per session
...
function getSessionHistory(sessionId, opts = { lastMinutes: 60 }) {
  const session = conversations[sessionId];
  if (!session || !session.history) return [];
  if (!opts.lastMinutes) return session.history.slice();
  const cutoff = nowMs() - (opts.lastMinutes * 60 * 1000);
  ...
  if (candidateIds.size === 0) {
    ...
    for (let i = 0; i < Math.min(MAX_GPT_SELLER_CHECK, sellersData.length) && candidateList.length < MAX_GPT_SELLER_CHECK; i++) {
      ...
      if (detectedGender) {
        ...
        if (sellerHasGender) {
          const genderMatch = detectedGender === 'men' ? categories.some(c => /\bmen\b|\bman\b|\bmens\b/.test(c))
                          : detectedGender === 'women' ? categories.some(c => /\bwomen\b|\bwoman\b|\bwomens\b|ladies/.test(c))
                          : categories.some(c => /\bkid\b|\bkids\b|\bchild\b|\bchildren\b/.test(c));
...
/* -------------------------
   ...
   (kept unchanged; this function expects conversationHistory array when provided)
--------------------------*/
async function getChatGPTResponse(userMessage, conversationHistory = [], companyInfo = ZULU_CLUB_INFO) {
  if (!process.env.OPENAI_API_KEY) {
    return "Hello! I'm here to help you with Zulu Club. Currently, I'm experiencing technical difficulties. Please visit zulu.club or contact our support team for assistance.";
...
/* -------------------------
   Webhook + endpoints
   (handleMessage now uses session/back-history helpers)
--------------------------*/
async function handleMessage(sessionId, userMessage) {
  try {
    // ensure session exists and append user message with timestamp
    appendToSessionHistory(sessionId, 'user', userMessage);

    // get session-limited history (last 60 minutes) to pass to GPT functions that accept conversationHistory
    const sessionHistory = getSessionHistory(sessionId, { lastMinutes: 60 });

    // pass sessionHistory into getChatGPTResponse so GPT can use prior messages
    const aiResponse = await getChatGPTResponse(userMessage, sessionHistory);

    // append assistant response
    appendToSessionHistory(sessionId, 'assistant', aiResponse);

    // trim session history to last 60 minutes and cap by MAX_HISTORY_MESSAGES
